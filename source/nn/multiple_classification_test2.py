...
np.random.seed(1)
data_train = pd.read_csv('./data/classification/data.circles.test.10000.csv')
data_test = pd.read_csv('./data/classification/data.circles.train.10000.csv')
...
layers.append(Layer(15, ReluFunction()))
layers.append(Layer(4, SoftMaxFunction()))

multilayerPerceptron = MultilayerPerceptron(layers, CrossEntropy(),weight_scale=0.5)
costs = multilayerPerceptron.train(x_train, y_train, 25000, 10000 , 0.1 ,0.2)
print("costs=")
#print(costs)
...
plot.plot_multipl_decision_region(x_train[:,0:1000],y_train[:,0:1000],4,multilayerPerceptron)
plot.plot_multipl_decision_region(x_test[:,0:1000],y_test[:,0:1000],4,multilayerPerceptron)
