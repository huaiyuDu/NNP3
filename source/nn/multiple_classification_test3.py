...
np.random.seed(1)
data_train = pd.read_csv('./data/classification/data.noisyXOR.train.1000.csv')
data_test = pd.read_csv('./data/classification/data.noisyXOR.test.1000.csv')
...
#layers.append(Layer(7, ReluFunction()))
layers.append(Layer(10, ReluFunction()))
layers.append(Layer(2, SoftMaxFunction()))

multilayerPerceptron = MultilayerPerceptron(layers, CrossEntropy(),weight_scale=0.2)
costs = multilayerPerceptron.train(x_train, y_train, 5000, 3000 , 0.2,0.2 )
print("costs=")
#print(costs)
...
plot.plot_multipl_decision_region(x_train[:,0:1000],y_train[:,0:1000],2,multilayerPerceptron)
plot.plot_multipl_decision_region(x_test[:,0:1000],y_test[:,0:1000],2,multilayerPerceptron)
